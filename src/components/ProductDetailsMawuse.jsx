...
const ProductDetailsMawuse = () => {
	...
	const handleDelete = async () => {
		...
		if (confirmDelete) {
			try {
				const deleted = await apiDeleteProduct(productID);
				console.log("del", deleted); // Call API to delete the product
				alert("Product deleted successfully.");
				navigate("/vendordashboard"); // Redirect to product list after deletion
			} catch (error) {
					...
					<div className="flex gap-5">
						...
						<div className="flex flex-col gap-5">
							<button
								...
								className="bg-green-500 text-white w-32 py-2 rounded-lg"
							>
								Edit
							</button>
							<button
